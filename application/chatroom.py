...
def get_messages():
    incoming = request.get_json()
    res = dispatch(Message.get_messages_from_room_id(incoming['room_id']))
    messages = [{'user_id': row[1], 'sendTime': row[2], 'content': row[3]} for row in res]
    for message in messages:
        res = dispatch(User.get_username_with_user_id(message['user_id']))
